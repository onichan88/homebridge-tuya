...
class SimpleFanLightAccessory extends BaseAccessory {
    ...
    _registerCharacteristics(dps) {
        ...
        this.dpActive = this._getCustomDP(this.device.context.dpActive) || '60';
        this.dpRotationSpeed = this._getCustomDP(this.device.context.RotationSpeed) || '62';
        this.dpDirection = this._getCustomDP(this.device.context.Direction) || '63';
	...
	this.useColorTemperature = this._coerceBoolean(this.device.context.useColorTemperature, true);
        this.maxSpeed = parseInt(this.device.context.maxSpeed) || 7;
	 ...
	 const characteristicDirection = serviceFan.getCharacteristic(Characteristic.Direction)
            .setProps({
                minValue: 0,
                maxValue: 1,
                minStep: 1
            })
            .updateValue(this._getSpeed(dps[this.dpDirection]))
            .on('get', this.getSpeed.bind(this))
            .on('set', this.setSpeed.bind(this));
	...
	let characteristicColorTemperature;
        ...
        this.device.on('change', (changes, state) => {
            ...
            if (changes.hasOwnProperty(this.dpDirection) && characteristicDirection.value !== changes[this.dpDirection])
                characteristicDirection.updateValue(changes[this.dpDirection]);
	    ...
	    if (changes.hasOwnProperty(this.dpColorTemperature) && characteristicColorTemperature && characteristicColorTemperature.value !== changes[this.dpColorTemperature])
                characteristicColorTemperature.updateValue(changes[this.dpColorTemperature]);
...
// Fan Direction
    getDirection(callback) {
        this.getState(this.dpDirection, (err, dp) => {
            if (err) return callback(err);

            callback(null, this._getDirection(dp));
        });
    }
...
//		console.log("_getSpeed = " + dp);
    ...
    setDirection(value, callback) {
        const {Characteristic} = this.hap;
        if (value == 0) {
        	return this.setState(this.dpActive, false, callback);
        } else {
        	return this.setState(this.dpDirection, value.toString(), callback);
        }
...
// Lightbulb ColorTemperature
    ...
    _getColorTemperature(dp) {
        const {Characteristic} = this.hap;
//		console.log("_getColorTemperature = " + dp);
        return dp;
    }

    setColorTemperature(value, callback) {
        const {Characteristic} = this.hap;
//        console.log("setColorTemperature - Raw value = " + value);
        return this.setState(this.dpColorTemperature, value, callback);
